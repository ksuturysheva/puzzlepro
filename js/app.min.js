...
function popupsInit(newPopup){
	...
	jQuery(".page-width").css("min-height","100%");var bodyH = jQuery(window).outerHeight();var popupH = jQuery(newPopup).outerHeight();

	var popupTop = 0;if( popupH < bodyH ){popupTop = (bodyH-popupH)/2 - 50;}if(popupTop < 0){popupTop = 0;}
...
$(document).ready(function() {
	...
	jQuery('.services-text').html(jQuery('.services-block__item.active').find('.text').html())
	jQuery(document).on("click",".services-block__item", function(){
		jQuery('.services-block__item').removeClass("active") 
		jQuery(this).addClass("active") 
		jQuery('.services-text').html(jQuery(this).find('.text').html())
	});

	jQuery(document).on("click",".project-nav li", function(){
		jQuery('.project-nav li').removeClass("active") 
		jQuery(this).addClass("active")
		jQuery('.projects-item').removeClass("active") 
		jQuery('.projects-item.' + jQuery(this).data('filter')+ '').addClass("active") 
	});

	jQuery(document).on("click",".projects-item", function(){
		jQuery('.projects-item').removeClass("active-plus") 
		jQuery(this).addClass("active-plus")
	});

	jQuery(document).on("click",".rotation", function(){
		jQuery(this).parents('.team-item').toggleClass("active")
	});

	jQuery(document).on("click",".quiz .btns .next", function(){
		var idstep = jQuery('.step.active').index() + 2;
		jQuery('.step').removeClass('active');
		jQuery('#step-' + parseInt(idstep) + '').addClass("active");
		if (idstep == '6') {jQuery('.quiz>.btns').hide();}
	...
	jQuery(document).on("click", ".popup-link", function(){
		...
		else{jQuery(".popup-bl").removeClass("show");
		jQuery(newPopup).toggleClass("show");}
	...
	jQuery(document).on("click", ".popup-bl .close,.popup-bl .btn-close", function(){
		...
		jQuery(".woocommerce-message").remove(0);
		return false;
	});

	jQuery( "#tabs" ).tabs();

	var wooSelLen = jQuery(".select").length;
	for(var i=0;i<wooSelLen;i++)
	{
		var wooSel = jQuery(".select").eq(i);
	...
	jQuery(document).on("click",".select-list .title", function(){
		var newSel = jQuery(this).parent();
		if( jQuery(newSel).hasClass("open") != true ){jQuery(newSel).find("ul").show(200);jQuery(newSel).toggleClass("open");}
		else{jQuery(newSel).find("ul").hide(0);jQuery(newSel).removeClass("open");}
	});
	jQuery(document).on("click",".select-list li span", function(){
		var newSel = jQuery(this).parent().parent().parent();
		var newVal = jQuery(this).html();
		jQuery(newSel).find(".title span").html(newVal);
		var selVal = jQuery(this).data("value");
		jQuery(newSel).find("input[type='hidden']").val(selVal);
		jQuery(newSel).find("ul .active").removeClass("active");
		jQuery(this).parent().toggleClass("active");
		jQuery(newSel).find("ul").hide(0);
		jQuery(newSel).removeClass("open");

		var newBl = jQuery(this).parent().parent().parent().parent();
		var ind = jQuery(this).parent().index();
		jQuery(newBl).find("select option:selected").prop('selected', false);
		jQuery(newBl).find("select option").eq(ind).prop('selected', true);
	});

	const swiperscroll = new Swiper(".mySwiper", {
		spaceBetween:10,
		slidesPerView:'auto',
		freeMode:true,
    ...
    const swiperscroll2 = new Swiper(".mySwiper2", {
		spaceBetween:20,
		slidesPerView:'auto',
		freeMode:true,
		watchSlidesVisibility:true,
